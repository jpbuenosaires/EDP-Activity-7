...
namespace EcommerceIS
{
    public class FrmPasswordRecovery : Form
    {
        ...
        private void BuildStep1Panel()
        {
            ...
            this.txtEmail = MakeTextBox(new Point(30, 100), "admin@ecommerce.local");
        ...
        private void BuildStep2Panel()
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                txtCode[i].TextChanged += (s, e) =>
                {
                    if (txtCode[idx].Text.Length == 1 && idx < 5)
                    {
                        txtCode[idx + 1].Focus();
                    }
                };

                txtCode[i].KeyDown += (s, e) =>
                {
                    if (e.KeyCode == Keys.Back && txtCode[idx].Text.Length == 0 && idx > 0)
                    {
                        txtCode[idx - 1].Focus();
                    }
                };
            ...
            lnkRes.LinkClicked += (s, e) => SendVerificationCode();
            ...
            { 
                string code = "";
                foreach (var tb in txtCode) code += tb.Text;
                if (code == generatedOTP && !string.IsNullOrEmpty(generatedOTP)) 
                {
        ...
        private void BuildStep3Panel()
        {
            ...
            this.txtNewPass  = MakeTextBox(new Point(30, 100), "");
            this.txtNewPass.PasswordChar = '●';
            this.txtNewPass.Size         = new Size(450, 30);

            var lblConf = MakeFieldLabel("CONFIRM PASSWORD", new Point(30, 145));
            this.txtConfirmPass = MakeTextBox(new Point(30, 165), "");
            this.txtConfirmPass.PasswordChar = '●';
            this.txtConfirmPass.Size         = new Size(450, 30);

            var chkShowPass = new CheckBox
            {
                Text = "👁",
                Font = new Font("Segoe UI", 12f),
                Location = new Point(485, 130),
                AutoSize = true,
                Cursor = Cursors.Hand
            };
            chkShowPass.CheckedChanged += (s, e) =>
            {
                this.txtNewPass.PasswordChar = chkShowPass.Checked ? '\0' : '●';
                this.txtConfirmPass.PasswordChar = chkShowPass.Checked ? '\0' : '●';
            };
            ...
            btnRst.Click += (s, e) =>
            {
                ...
                if (!System.Text.RegularExpressions.Regex.IsMatch(this.txtNewPass.Text, @"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).{8,}$"))
                {
                ...
                try 
                {
                    if (UserService.ResetPassword(this.txtEmail.Text.Trim(), this.txtNewPass.Text))
                    {
            ...
            pnlStep3.Controls.AddRange(new Control[]
                { lblInfo, lblNew, this.txtNewPass, lblConf, this.txtConfirmPass, chkShowPass, btnRst });
        }
        ...
        private void SendVerificationCode()
        {
            ...
            try
            {
                if (!UserService.CheckEmailExists(this.txtEmail.Text.Trim()))
                {
                ...
                {
                    Port = 587,
                    Credentials = new System.Net.NetworkCredential("jpb2023-7457-37456@bicol-u.edu.ph", "jstw lzcx yirm mziu"),
                    EnableSsl = true,
                };

                var mailMessage = new System.Net.Mail.MailMessage
                {
                    From = new System.Net.Mail.MailAddress("jpb2023-7457-37456@bicol-u.edu.ph"),
                    Subject = "Your Password Reset Code",
                    Body = $"Your 6-digit verification code is: {generatedOTP}",
                    IsBodyHtml = false,
                };
                mailMessage.To.Add(this.txtEmail.Text.Trim());
                ...
                {
                    MessageBox.Show($"Failed to send email. (SMTP not configured).\nFor testing purposes, your code is: {generatedOTP}", "Demo Mode", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
