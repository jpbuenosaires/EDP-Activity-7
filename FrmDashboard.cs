...
namespace EcommerceIS
{
    ...
    {
        public static string LoggedInUser = "Administrator";
        public static string LoggedInRole = "Admin";
        public static string LoggedInEmail = "admin@ecommerce.local";
        ...
        private void SetActiveMenu(string menuName)
        {
            ...
            {
                if (c is Button btn && !btn.Text.Contains("Logout"))
                {
                    bool isActive = btn.Text.Contains(menuName);
                    btn.Font = new Font("Segoe UI", 9.5f, isActive ? FontStyle.Bold : FontStyle.Regular);
                    btn.ForeColor = isActive ? Color.White : Color.FromArgb(180, 200, 230);
                    btn.BackColor = isActive ? Color.FromArgb(46, 90, 170) : Color.Transparent;
                }
            }
        }
        ...
        public void UpdateTopBar(string title, string subtitle)
        {
            ...
            if (title != "Dashboard") {
                lblGreeting.Text = title;
                lblDateTime.Text = subtitle;
            } else {
                string hour = DateTime.Now.Hour < 12 ? "morning" :
                              DateTime.Now.Hour < 18 ? "afternoon" : "evening";
                lblGreeting.Text = $"Good {hour}, {LoggedInUser} 👋";
        ...
        private void BuildSidebar()
        {
            ...
            var navList = new System.Collections.Generic.List<string[]>();
            navList.Add(new[] { "📊", "Dashboard",       "true"  });
            navList.Add(new[] { "🛒", "Sales",            "false" });
            navList.Add(new[] { "📋", "Order Processing", "false" });
            navList.Add(new[] { "📦", "Inventory",        "false" });
            navList.Add(new[] { "👥", "Customers",        "false" });
            if (LoggedInRole == "Admin") navList.Add(new[] { "🏷️",  "Categories",      "false" });
            if (LoggedInRole == "Admin") navList.Add(new[] { "📈", "Report Generator", "false" });
            if (LoggedInRole == "Admin") navList.Add(new[] { "🧑‍💼", "User Management",  "false" });
            string[][] navItems = navList.ToArray();
            ...
            foreach (var item in navItems)
            {
                ...
                if (item[1] == "Dashboard")
                    btn.Click += (s, e) => { SetActiveMenu("Dashboard"); UpdateTopBar("Dashboard", ""); LoadDashboardContent(); };
                else if (item[1] == "Sales")
                    btn.Click += (s, e) =>
                    {
                        SetActiveMenu("Sales");
                        UpdateTopBar("🛒  Sales Transaction", "Process new sales orders for customers.");
                        pnlContent.Controls.Clear();
                        var frm = new FrmSalesTransaction();
                        frm.TopLevel = false;
                ...
                else if (item[1] == "Order Processing")
                    btn.Click += (s, e) =>
                    {
                        SetActiveMenu("Order Processing");
                        UpdateTopBar("📋  Order Processing", "View and manage customer orders, update order status.");
                        pnlContent.Controls.Clear();
                        var frm = new FrmOrderProcessing();
                        frm.TopLevel = false;
                        frm.FormBorderStyle = FormBorderStyle.None;
                        frm.Dock = DockStyle.Fill;
                        pnlContent.Controls.Add(frm);
                ...
                else if (item[1] == "Inventory")
                    btn.Click += (s, e) =>
                    {
                        SetActiveMenu("Inventory");
                        UpdateTopBar("📦  Inventory Management", "Manage product stock levels, add new products, and restock.");
                        pnlContent.Controls.Clear();
                        var frm = new FrmInventoryManagement();
                        frm.TopLevel = false;
                        frm.FormBorderStyle = FormBorderStyle.None;
                        frm.Dock = DockStyle.Fill;
                        pnlContent.Controls.Add(frm);
                ...
                else if (item[1] == "Report Generator")
                    btn.Click += (s, e) =>
                    {
                        SetActiveMenu("Report Generator");
                        UpdateTopBar("📈  Report Generator", "Generate, filter, and export reports from the EcommerceDB database.");
                        pnlContent.Controls.Clear();
                        var frm = new FrmReportGenerator();
                        frm.TopLevel = false;
                        frm.FormBorderStyle = FormBorderStyle.None;
                        frm.Dock = DockStyle.Fill;
                        pnlContent.Controls.Add(frm);
                ...
                else if (item[1] == "User Management")
                    btn.Click += (s, e) =>
                    {
                        SetActiveMenu("User Management");
                        UpdateTopBar("🧑‍💼  User Management", "Manage user accounts, roles, and system access.");
                        pnlContent.Controls.Clear();
            ...
            foreach (var item in accountItems)
            {
                ...
                if (item[1] == "About")
                    ...
                    {
                        SetActiveMenu("About");
                        UpdateTopBar("ℹ️  About System", "Information and details about EcommerceDB System.");
                        pnlContent.Controls.Clear();
            ...
            pnlUser.Paint += (s, e) =>
            {
                ...
                string initial = string.IsNullOrEmpty(LoggedInUser) ? "U" : LoggedInUser.Substring(0, 1).ToUpper();
                using (var f = new Font("Segoe UI", 14f, FontStyle.Bold))
                using (var br = new SolidBrush(Color.White))
                    g.DrawString(initial, f, br, 26, 20);

                using (var f = new Font("Segoe UI", 9.5f, FontStyle.Bold))
                using (var br = new SolidBrush(Color.White))
                    g.DrawString(LoggedInRole, f, br, 62, 18);
                using (var f2 = new Font("Segoe UI", 8f))
                using (var br2 = new SolidBrush(Color.FromArgb(140, 170, 210)))
                    g.DrawString(LoggedInEmail, f2, br2, 62, 37);
            };
        ...
        private void BuildTopBar()
        {
            ...
            {
                Text      = $"Good morning, {LoggedInUser} 👋",
        ...
        private void BuildStatBar()
        {
            ...
            pnlStat.Paint += (s, e) =>
            {
                ...
                using (var br = new SolidBrush(Color.FromArgb(100, 110, 140)))
                {
                    ...
                    g.DrawString($"👤  Logged in: {LoggedInUser}", f, br, 420, 16);
